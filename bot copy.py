...
def init_db():
    ...
    # Cập nhật DB cho tính năng Workspace & Topic
...
def sync_tickets_to_new_group(current_bot, new_group_id):
    ...
    try:
        ...
        print(f"❌ Không thể gửi thông báo vào nhóm mới {new_group_id}: {e}")
        conn.close()
        return
                    ...
                    if bot and GROUP_IT_ID:
                        if status == 'Hoàn thành':
                            ...
                            if g_msg_id:
                                ...
                                # Gỡ ghim tin nhắn
                                try: bot.unpin_chat_message(GROUP_IT_ID, g_msg_id)
                                except: pass
                                        ...
                                        if role in ['main', 'support']:
                                            ...
                                            if target_topic:
                                                ...
                                                if ws_row and ws_row[0]:
                                                    try: bot.edit_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic, name=f"✅ [ĐÃ XONG] #{t_id} - {row[2]}"[:120])
                                                    except: pass
                                                    try: bot.send_message(ws_row[0], f"Ticket đã được đóng từ Web Dashboard.", message_thread_id=target_topic)
                                                    except: pass
                                                    try: bot.close_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic)
                                                    except: pass
                                            else:
                                                try: bot.send_message(p_id, f"🎉 Ticket **#{t_id}** đã được đóng từ Web Dashboard.")
                                                except: pass
                                        elif role == 'customer':
                                            try: bot.send_message(p_id, f"✅ **Sự cố của bạn đã hoàn tất.**\nVui lòng đánh giá dịch vụ:", reply_markup=get_rating_keyboard(t_id), parse_mode="Markdown")
                                            except: pass
                                            try: bot.send_message(p_id, "👇 Báo sự cố khác:", reply_markup=get_report_keyboard())
                                            except: pass
                                    except: pass
                            else:
                                # Fallback nếu active_sessions đã bị xóa nhưng DB đổi thành hoàn thành
                                if topic_id_db and it_id_db:
                                    cursor.execute("SELECT workspace_group_id FROM it_staff WHERE it_id = ?", (it_id_db,))
                                    ws_row = cursor.fetchone()
                                    if ws_row and ws_row[0]:
                                        try: bot.edit_forum_topic(chat_id=ws_row[0], message_thread_id=topic_id_db, name=f"✅ [ĐÃ XONG] #{t_id} - {row[2]}"[:120])
                                        except: pass
                                        try: bot.send_message(ws_row[0], f"Ticket đã được đóng từ Web Dashboard.", message_thread_id=topic_id_db)
                                        except: pass
                                        try: bot.close_forum_topic(chat_id=ws_row[0], message_thread_id=topic_id_db)
                                        except: pass
                            ...
                            if g_msg_id:
                                ...
                                # Ghim Ticket bị trả lại
                                try: bot.pin_chat_message(GROUP_IT_ID, g_msg_id, disable_notification=True)
                                except: pass
                                    ...
                                    try:
                                        if role in ['main', 'support']:
                                            ...
                                            if target_topic:
                                                ...
                                                ws_row = cursor.fetchone()
                                                if ws_row and ws_row[0]:
                                                    try: bot.edit_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic, name=f"❌ [TRẢ LẠI] #{t_id} - {row[2]}"[:120])
                                                    except: pass
                                                    try: bot.send_message(ws_row[0], f"Ticket đã bị Quản lý hủy nhận việc từ Web Dashboard.", message_thread_id=target_topic)
                                                    except: pass
                                                    try: bot.close_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic)
                                                    except: pass
                                            else:
                                                try: bot.send_message(p_id, f"🔙 Ticket **#{t_id}** đã bị Quản lý hủy nhận việc từ Web Dashboard.")
                                                except: pass
                                        elif role == 'customer':
                                            try: bot.send_message(p_id, "⚠️ IT hiện tại đang bận, sự cố của bạn đã được chuyển lại cho team. Sẽ có IT khác tiếp nhận ngay!")
                                            except: pass
                                    except: pass
                            else:
                                # Fallback nếu active sessions rỗng
                                if topic_id_db and it_id_db:
                                    cursor.execute("SELECT workspace_group_id FROM it_staff WHERE it_id = ?", (it_id_db,))
                                    ws_row = cursor.fetchone()
                                    if ws_row and ws_row[0]:
                                        try: bot.edit_forum_topic(chat_id=ws_row[0], message_thread_id=topic_id_db, name=f"❌ [TRẢ LẠI] #{t_id} - {row[2]}"[:120])
                                        except: pass
                                        try: bot.send_message(ws_row[0], f"Ticket đã bị Quản lý hủy nhận việc từ Web Dashboard.", message_thread_id=topic_id_db)
                                        except: pass
                                        try: bot.close_forum_topic(chat_id=ws_row[0], message_thread_id=topic_id_db)
                                        except: pass
...
def setup_bot_handlers(current_bot):
    ...
    def handle_all_messages(message):
        ...
        # --- TRƯỜNG HỢP 1: TIN NHẮN TỪ NHÓM WORKSPACE CỦA IT (TRONG TOPIC) ---
        if message.chat.type in ['group', 'supergroup']:
            if message.message_thread_id:
                conn = connect_db(); cursor = conn.cursor()
                cursor.execute("SELECT ticket_id, role, user_id FROM active_sessions WHERE topic_id = ?", (message.message_thread_id,))
                ...
                if sess:
                    ...
                    for p_id, p_role, p_topic in cursor.fetchall():
                        ...
                        try:
                            if message.content_type == 'text': current_bot.send_message(target_chat, f"{prefix}{message.text}", parse_mode="Markdown", message_thread_id=target_thread)
                            elif message.content_type == 'photo': current_bot.send_photo(target_chat, message.photo[-1].file_id, caption=f"{prefix}{message.caption or ''}", parse_mode="Markdown", message_thread_id=target_thread)
                            elif message.content_type == 'document': current_bot.send_document(target_chat, message.document.file_id, caption=f"{prefix}{message.caption or ''}", parse_mode="Markdown", message_thread_id=target_thread)
                            elif message.content_type == 'video': current_bot.send_video(target_chat, message.video.file_id, caption=f"{prefix}{message.caption or ''}", parse_mode="Markdown", message_thread_id=target_thread)
                            elif message.content_type == 'voice': current_bot.send_voice(target_chat, message.voice.file_id, caption=f"{prefix}{message.caption or ''}", parse_mode="Markdown", message_thread_id=target_thread)
                            elif message.content_type == 'audio': current_bot.send_audio(target_chat, message.audio.file_id, caption=f"{prefix}{message.caption or ''}", parse_mode="Markdown", message_thread_id=target_thread)
                        except: pass
                conn.close()
            return
        
        # --- TRƯỜNG HỢP 2: TIN NHẮN TỪ CHAT RIÊNG (PRIVATE) ---
        if message.chat.type != 'private': return
        ...
        # Xóa hẳn nút Hủy ở tin nhắn cũ đi ngay khi có tin nhắn mới (tránh KH bấm nhầm lại)
        if temp_data:
        ...
        # Ghim Message mới vào Nhóm IT Tổng
        if sent_msg:
            try: current_bot.pin_chat_message(GROUP_IT_ID, sent_msg.message_id, disable_notification=True)
            except: pass
        ...
        if action == 'claim':
            ...
            if workspace_id:
                try:
                    ...
                    # Ghim nội dung ticket bên trong Topic để dễ nhìn
                    try: current_bot.pin_chat_message(workspace_id, sent_it_msg.message_id, disable_notification=True)
                    except: pass
                except Exception as e:
            ...
            # Gỡ ghim tin nhắn khi IT đã nhận việc ở nhóm chung
            try: current_bot.unpin_chat_message(GROUP_IT_ID, call.message.message_id)
            except: pass
            
            current_bot.send_message(user_id, f"👨‍💻 IT **{it_info[0]}** đang hỗ trợ bạn. Vui lòng giữ kết nối.", parse_mode="Markdown")
            
            if workspace_id:
                current_bot.answer_callback_query(call.id, f"✅ Đã nhận việc! Topic #{ticket_id} đã mở.")
            else:
                current_bot.answer_callback_query(call.id, "✅ Nhận việc thành công!")
            ...
            if workspace_id:
                try:
                    ...
                    # Ghim nội dung vào phòng Topic
                    try: current_bot.pin_chat_message(workspace_id, sent_sup_msg.message_id, disable_notification=True)
                    except: pass
                except Exception as e:
                    workspace_id = None
            ...
            # Gỡ ghim tin nhắn yêu cầu hỗ trợ sau khi có người nhận
            try: current_bot.unpin_chat_message(GROUP_IT_ID, call.message.message_id)
            except: pass
                ...
                try:
                    if role == 'customer': current_bot.send_message(p_id, f"👨‍🔧 IT **{it_info[0]}** vừa tham gia hỗ trợ.", parse_mode="Markdown")
                        ...
                        if p_topic:
                            ...
                            ws = cursor.fetchone()
                            if ws and ws[0]:
                                target_chat, target_thread = ws[0], p_topic
            ...
            # Tách riêng các luồng try-except để không bị nghẽn do giới hạn API Telegram
            ...
            if res and res[3]: # group_support_msg_id
                try: current_bot.delete_message(GROUP_IT_ID, res[3])
                except: pass
            if res and res[6]: # unpin cũ (nếu có)
                try: current_bot.unpin_chat_message(GROUP_IT_ID, res[6])
                except: pass
            
            try: current_bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=None)
            except: pass
            
            # Gửi thông báo hủy vào đúng Topic cho từng IT với try-except rời rạc
            for p_id, role, p_topic in participants:
                if role in ['main', 'support']:
                    if p_topic:
                        cursor.execute("SELECT workspace_group_id FROM it_staff WHERE it_id = ?", (p_id,))
            ...
            # Lấy thông tin ID Topic của người làm chính lưu trong DB (res[8] thay thế)
            ...
            if res and res[6]: # group_support_msg_id
                try: current_bot.delete_message(GROUP_IT_ID, res[6])
                except: pass
            
            if res and res[7]: # group_msg_id (Gỡ ghim trên nhóm Tổng)
                try: current_bot.unpin_chat_message(GROUP_IT_ID, res[7])
                except: pass

            # Đóng Topic: Chia rẽ các khối lệnh Edit / Send / Close để tránh kẹt lệnh Telegram
            for p_id, role, p_topic in participants:
                if role in ['main', 'support']:
