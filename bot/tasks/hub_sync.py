"""
Background Task: Hub & Group Synchronization
Monitors Web Dashboard ticket status changes and updates Telegram group messages and forum topics in real time.
"""
import time
from telebot import types
import config.settings as bot_config
from database.connection import connect_db
from utils.helpers import safe_edit_message, truncate_text
...
def sync_tickets_to_new_group(current_bot, new_group_id):
    if not current_bot or not new_group_id:
        return
    conn = connect_db()
    try:
        cursor = conn.cursor()
        ...
        try:
            current_bot.send_message(new_group_id, "🔄 **HỆ THỐNG ĐANG ĐỒNG BỘ DỮ LIỆU...**", parse_mode="Markdown")
        ...
        for r in active_tickets:
            ...
            try:
                if status == 'Mới':
                    ...
                    if sent_msg:
                        try:
                            current_bot.pin_chat_message(chat_id=new_group_id, message_id=sent_msg.message_id, disable_notification=True)
                        except Exception:
                            pass
                ...
                print(f"⚠️ Lỗi khi bắn Ticket #{t_id}: {ex}")
                            ...
                            if status == 'Hoàn thành':
                                if status_changed:
                                    if it_id_db and it_msg_id_db:
                                        try:
                                            bot_config.bot.edit_message_reply_markup(chat_id=it_id_db, message_id=it_msg_id_db, reply_markup=None)
                                        except Exception:
                                            pass
                                    if g_msg_id:
                                        sup_text = f"\n👨‍🔧 **Hỗ trợ:** {support_it_names}" if support_it_names else ""
                                        ...
                                        try:
                                            bot_config.bot.unpin_chat_message(chat_id=bot_config.GROUP_IT_ID, message_id=g_msg_id)
                                        except Exception:
                                            pass
                                            ...
                                            if role in ['main', 'support']:
                                                ...
                                                if target_topic:
                                                    ...
                                                    if ws_row and ws_row[0]:
                                                        try:
                                                            bot_config.bot.edit_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic, name=f"✅ [ĐÃ XONG] #{t_id} - {user_name}"[:120])
                                                        except Exception:
                                                            pass
                                                        try:
                                                            bot_config.bot.send_message(ws_row[0], f"Ticket đã được đóng từ Web Dashboard.", message_thread_id=target_topic)
                                                        except Exception:
                                                            pass
                                                        try:
                                                            bot_config.bot.close_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic)
                                                        except Exception:
                                                            pass
                                                else:
                                                    try:
                                                        bot_config.bot.send_message(p_id, f"🎉 Ticket **#{t_id}** đã được đóng từ Web Dashboard.")
                                                    except Exception:
                                                        pass
                                            elif role == 'customer':
                                                try:
                                                    bot_config.bot.send_message(p_id, f"✅ **Sự cố của bạn đã hoàn tất.**\nVui lòng đánh giá dịch vụ:", reply_markup=get_rating_keyboard(t_id), parse_mode="Markdown")
                                                except Exception:
                                                    pass
                                                try:
                                                    bot_config.bot.send_message(p_id, "👇 Báo sự cố khác:", reply_markup=get_report_keyboard())
                                                except Exception:
                                                    pass
                                        except Exception:
                                            pass
                                ...
                                if status_changed:
                                    ...
                                    if g_msg_id:
                                        ...
                                        try:
                                            bot_config.bot.pin_chat_message(chat_id=bot_config.GROUP_IT_ID, message_id=g_msg_id, disable_notification=True)
                                        except Exception:
                                            pass
                                    ...
                                    if participants:
                                            ...
                                            try:
                                                if role in ['main', 'support']:
                                                    ...
                                                    if target_topic:
                                                        ...
                                                        if ws_row and ws_row[0]:
                                                            try:
                                                                bot_config.bot.edit_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic, name=f"❌ [TRẢ LẠI] #{t_id} - {user_name}"[:120])
                                                            except Exception:
                                                                pass
                                                            try:
                                                                bot_config.bot.send_message(ws_row[0], f"Ticket đã bị Quản lý hủy từ Web.", message_thread_id=target_topic)
                                                            except Exception:
                                                                pass
                                                            try:
                                                                bot_config.bot.close_forum_topic(chat_id=ws_row[0], message_thread_id=target_topic)
                                                        ...
                                                        try:
                                                            bot_config.bot.send_message(p_id, f"🔙 Ticket **#{t_id}** đã bị Quản lý hủy từ Web.")
                                                        except Exception:
                                                            pass
                                                elif role == 'customer':
                                                    try:
                                                        bot_config.bot.send_message(p_id, "⚠️ IT đang bận, sự cố đã chuyển lại cho team!")
                                                    except Exception:
                                                        pass
                                            except Exception:
                                                pass
                                        ...
                                        if topic_id_db and it_id_db:
                                            ...
                                            if ws_row and ws_row[0]:
                                                try:
                                                    bot_config.bot.edit_forum_topic(chat_id=ws_row[0], message_thread_id=topic_id_db, name=f"❌ [TRẢ LẠI] #{t_id} - {user_name}"[:120])
                                                except Exception:
                                                    pass
                                                try:
                                                    bot_config.bot.send_message(ws_row[0], f"Ticket bị hủy từ Web.", message_thread_id=topic_id_db)
                                                except Exception:
                                                    pass
                                                try:
                                                    bot_config.bot.close_forum_topic(chat_id=ws_row[0], message_thread_id=topic_id_db)
                                                except Exception:
                                                    pass
                                else:
                                    if g_msg_id:
